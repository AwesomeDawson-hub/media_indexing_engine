...
### ADR-032: P9-003 Introduces Item-Owned Origin and Preview Records Without Replacing `SourceObject` or `MediaItem`
- **Date:** 2026-04-09
- **Workstream:** P9-003
- **Status:** Accepted
- **Context:** After P9-001 and P9-002 completed, the next open design question was how to split origin and preview concerns out of `MediaItem` without destabilizing the live beta schema. `SourceObject` already existed as connector sync memory, `MediaItem` still owned `storage_path`, `thumbnail_path`, and `source_file_fingerprint`, and the upcoming P9-004 write-back work needed a stable item-owned origin locator rather than continued reliance on a mixed `MediaItem` plus `SourceObject` field contract.
- **Decision:** P9-003 adds two new additive tables: `OriginAssetRef` and `PreviewAsset`. `OriginAssetRef` is a new 1:1 child of `MediaItem` and is the canonical item-owned origin locator for all media items, including connector-backed items, local-folder items, and manual app-retained uploads. `SourceObject` is not replaced; it remains connector sync memory. `PreviewAsset` becomes the canonical retained preview record. `MediaItem.storage_path`, `MediaItem.thumbnail_path`, and `MediaItem.source_file_fingerprint` become true migrations with compatibility mirrors during rollout. Mutation/write-back fields remain on `MediaItem` in P9-003 and are not moved until a later workstream.
- **Reasoning:** This is the smallest additive split that gives the system a stable, item-owned origin contract before introducing durable write-back operations. Reusing `SourceObject` as the canonical origin model would keep origin identity source-owned and connector-specific, which would not generalize to local-folder or app-upload items. Moving mutation/write-back state in the same slice would add unnecessary migration risk and blur the boundary with P9-004.
- **Alternatives considered:** Replace `SourceObject` with `OriginAssetRef` outright (rejected: destroys valid sync-memory semantics), limit P9-003 to `PreviewAsset` only (rejected: leaves P9-004 without the correct target contract for write-back operations), implement `WriteBackOperation` against existing `MediaItem` and `SourceObject` fields first (rejected: cements the current locator smear into another subsystem).
- **Consequences:** P9-003 is a prerequisite for P9-004 as currently defined. Engineer must add new tables, relationships, backfills, and compatibility read/write rules without dropping old columns in the same slice.

### ADR-033: Browser and Local Intake Must Become Working-Folder-First Rather Than Permanent App-Retained Upload Storage
- **Date:** 2026-04-10
- **Workstream:** P9-005
- **Status:** Accepted
- **Context:** Phase 9 closed the connector-ingestion, source-aware access, origin/preview split, and durable write-back gaps through P9-001 to P9-004. However, the clarified operator rule is that browser drag-drop and local intake must also stop creating permanent AWS-retained originals. The approved ARCH-002 architecture already states that browser/local intake should require a user-selected local working/export folder and must not silently fall back to permanent AWS copy storage. The current implementation still creates new retained-origin `app_upload` rows for ordinary browser/local intake.
- **Decision:** P9-005 will replace ordinary retained-original browser upload behavior with working-folder-first local intake. New browser/local items must be represented as `local_folder` source-aware reference-mode records rather than new `app_upload` retained-origin rows. Backend processing may still receive bytes transiently from the browser for analysis, preview generation, and indexing, but it must not persist new originals into app storage for this flow. Unsupported browsers must fail into an explicit unsupported/onboarding path rather than silently reverting to retained-upload storage.
- **Reasoning:** This is a real remaining ARCH-002 boundary gap, not optional cleanup. Requiring a selected local working/export folder keeps the user device as the source of truth for browser/local originals. Transient-byte processing preserves the existing analysis pipeline value without rebuilding permanent app retention. Explicit unsupported-browser behavior is better than silently violating the storage policy.
- **Alternatives considered:** Keep the current `app_upload` retained-original path for browser/local intake (rejected: violates the clarified operator rule and the ARCH-002 storage intent), silently fall back to AWS-retained uploads when browser file-system capabilities are missing (rejected: hidden policy violation), perform a destructive migration of historical `app_upload` rows now (rejected: no safe source-of-truth replacement and risk of data loss).
- **Consequences:** Phase 9 remains open with P9-005 as the current approval gate. Historical `app_upload` rows remain compatibility data, but Engineer must stop creating new ones for ordinary browser/local intake. Frontend intake preconditions and unsupported-browser behavior will change, and local-folder items will rely on source-aware re-confirmation rather than app-retained originals.

### ADR-034: P10-001 Uses Drive-Only Shared Refetch With Standardized Error Payloads and 502 Classification for Internal Inconsistency
- **Date:** 2026-04-12
- **Workstream:** P10-001
- **Status:** Accepted
- **Context:** The P10-001 implementation already landed in the repo before governance closeout was reconciled. The live code includes `src/connectors/drive_reference_fetch.py`, Drive-backed re-analysis integration in `src/api/routes/analysis.py`, Drive-backed download integration in `src/api/routes/download.py`, and dedicated tests in `tests/test_p10_001_drive_reference_fetch.py`. Auditor re-pass then found drift between the plan and the implementation: governance still described P10-001 as a pre-implementation approval gate, and the shared fetch service used `detail.error` plus `422` for service-level misuse or incomplete Drive reference state even though the project-standard API contract is `detail` plus `error_code`, and the Architect plan treated those cases as implementation inconsistency rather than client validation.
- **Decision:** P10-001 is no longer governed as a plan-stage approval gate. It is an implemented workstream under Architect-locked reconciliation and closeout. The narrow technical scope remains unchanged: Drive-only, single-item re-analysis, and single-item download/export only. Non-Drive reference items continue to receive `409 original_at_source` at the route layer and must not enter the shared Drive fetch service. Shared Drive fetch failures must use standardized `detail` plus `error_code` payloads. Shared-service misuse or inconsistent supposedly-Drive item state, including missing `OriginAssetRef`, non-Drive `OriginAssetRef`, missing `provider_object_id`, missing `source_id`, or missing `SourceConnector`, is classified as `502 drive_fetch_failed`, not `422`.
- **Reasoning:** The codebase already has a global error-handler contract and frontend client behavior built around `error_code`, so the Drive fetch slice should conform to that project standard rather than invent a parallel `detail.error` shape. `422` is appropriate for malformed client input; it is not the right classification for internal drift between route gating and an item that should already have a valid Drive-backed reference contract. Treating those cases as `502 drive_fetch_failed` keeps the blame on implementation/state integrity, preserves operator clarity, and avoids teaching clients that they supplied a bad request when the inconsistency is internal.
- **Alternatives considered:** Leave the current service payload shape (`detail.error`) in place and document it as a special case (rejected: violates the project-standard API contract), keep `422` for missing or incomplete Drive reference state (rejected: misclassifies internal integrity drift as client validation failure), reopen P10-001 as a fresh approval-gate plan despite implemented code (rejected: governance would remain detached from reality).
- **Consequences:** Governance moved P10-001 out of planned approval-gate state and into implementation reconciliation/closeout status, Engineer aligned `src/connectors/drive_reference_fetch.py` and the dedicated P10-001 tests to the locked contract, and P10-001 closed out on 2026-04-12. Future work should treat ADR-034 as the settled contract baseline rather than reopen P10-001 workflow.

### ADR-035: Capability-Aware Batch Reanalysis and Bulk Export Must Diverge After P10-001
- **Date:** 2026-04-12
- **Workstream:** P11-001
- **Status:** Accepted
- **Context:** The live batch routes still reflect pre-ARCH-002 assumptions. `POST /api/v1/media/reanalyze-batch` in `src/api/routes/analysis.py` silently skips items whose originals are not in app storage, and `POST /api/v1/media/download-batch` in `src/api/routes/download.py` remains a synchronous ZIP builder that also skips non-full items. After P10-001, Drive-backed single-item refetch is intentionally narrow and settled, but no equivalent product or operational contract exists for batch behavior.
- **Decision:** Batch reanalysis and bulk export are separate future workstreams. P11-001 is limited to capability-aware batch reanalysis. It may expand beyond full/app-retained items only through explicit per-item accepted/blocked/rejected reporting and only for Drive-backed `storage_mode='reference'` items that can be processed through an approved asynchronous queueing model. Local-folder reference items and future non-Drive providers remain out of scope unless a later dedicated workstream expands them. Bulk export is deferred into a separate later workstream, P11-002, and must not inherit a provider-neutral or synchronous mixed-selection ZIP contract by default.
- **Reasoning:** Batch reanalysis can reuse the existing analysis job model if the request boundary becomes capability-aware and item-scoped. Bulk export has a different risk envelope: ZIP packaging, artifact lifecycle, long request duration, memory accumulation, and upstream connector rate limiting. Combining both concerns into one workstream would blur two separate product contracts and push the design toward a weak mixed compromise. Silent skip behavior is not acceptable as the final UX for either route.
- **Alternatives considered:** Keep both batch routes unchanged and document silent skips as acceptable (rejected: poor UX and hides ineligibility), extend both batch routes together in a single workstream (rejected: export and reanalysis have materially different operational and product contracts), make provider-neutral batch refetch the default for all reference items (rejected: ARCH-002 and current connector maturity do not justify that assumption).
- **Consequences:** P11-001 was implemented and closed on 2026-04-12 using the capability-aware boundary defined here. The shipped route keeps explicit per-item outcomes, Drive-only async admission for reference items, and the privacy-preserving rejected `media_item_not_found` contract for both missing and unauthorized IDs. P11-002 remained separate from P11-001 and was subsequently activated under ADR-036 as the current approval gate for async connector-aware bulk export. Future planning and implementation must preserve the connector-specific, source-aware boundary rather than treating reference-mode items as generically fetchable in bulk.

### ADR-036: Bulk Export Requires a Dedicated Async Export-Job Boundary After P11-001
- **Date:** 2026-04-12
- **Workstream:** P11-002
- **Status:** Accepted
- **Context:** The live `POST /api/v1/media/download-batch` route in `src/api/routes/download.py` is still a synchronous ZIP builder that loads only app-accessible originals, silently skips ineligible items, and assembles the ZIP in memory. After P10-001 and P11-001, the system now has a narrow Drive-only single-item refetch contract and a separate capability-aware batch reanalysis contract, but no equivalent bulk-export contract that safely supports mixed full and Drive-backed reference selections.
- **Decision:** The final mixed-selection bulk export contract must use a dedicated async export-job boundary rather than extending the synchronous `POST /api/v1/media/download-batch` route. P11-002 introduces submission-time accepted/blocked/rejected reporting, runtime export-job results for accepted items, bounded Drive-aware execution, and temporary user-scoped export artifacts. Full/app-retained items and Drive-backed `storage_mode='reference'` items are eligible in this slice. Local-folder reference items and future non-Drive reference providers remain blocked unless a later dedicated workstream expands them.
- **Reasoning:** Bulk export has a materially different operational profile from single-item download and batch reanalysis: long-running ZIP assembly, mixed storage classes, upstream connector fetches, temporary artifact lifecycle, and download-after-completion behavior. Treating the synchronous ZIP endpoint as the final contract would preserve silent skip UX, request-thread work, and weak failure reporting. A dedicated async export-job model keeps mixed-selection export explicit, bounded, and source-aware.
- **Alternatives considered:** Extend the synchronous `POST /api/v1/media/download-batch` route into the final mixed-selection contract (rejected: request duration, memory pressure, and silent skip behavior remain wrong), make provider-neutral bulk fetch the default for all reference items (rejected: conflicts with ARCH-002 and current connector maturity), merge bulk export back into P11-001 (rejected: reanalysis and export remain distinct product and operational contracts).
- **Consequences:** P11-002 became the current approval gate for post-P11 planning, and ADR-036 remains the authoritative closeout contract after implementation. The legacy synchronous ZIP route remains compatibility behavior, not the target contract. Future implementation must preserve short-lived user-scoped export artifacts, explicit per-item reporting, bounded Drive fetch concurrency, and the locked exclusion of local-folder and non-Drive provider bulk export by default. Post-implementation closeout reconciliation on 2026-04-12 first found material drift against that locked contract: incomplete `export_no_eligible_items` detail payload, in-memory ZIP assembly, missing startup/lifespan cleanup for expired artifacts, and incomplete status promotion to `expired`. P11-002 was therefore reopened for a narrow remediation slice. Subsequent remediation brought the implementation back into alignment with ADR-036, and the final Auditor re-pass approved the package for formal closeout on 2026-04-13. The unrelated failure in `tests/test_google_drive_connector.py` remains explicitly out of scope for P11-002 and is not a regression in this workstream.
